...
`@qa-scenario` annotations on them, turns each scenario into a Given/When/Then
plan, sends an AI agent to look at the real staging page, and returns `pass`,
`fail`, `manual_review`, or `skip` — with the harness, not the agent, deciding
...
- Zero runtime dependencies. `@playwright/test` is an *optional* peer, needed only
  for the pre-authenticated session and the trace/HAR evidence it enables.
...
No credentials, no model, no network: it serves a bundled demo app on a local port
and runs the whole pipeline with the `fixture` adapter, whose judge always returns
`manual_review` so an offline run can never look green. Add `--keep` or
`--out=<dir>` to inspect the artifacts afterwards.
...
**① Your spec** — the annotations are the QA intent. Nothing here is executed
against staging.
...
**② `abstract-ai` → `dashboard-qa-spec-live.md`** — CI mock literals become
intent, because they cannot be replayed live.
...
**③ `judge` → `dashboard-hermes-judgment.md`** — the agent browses, the harness
scores.
...
| `login`       | Headed browser, log in once; `judge` reuses the session.               |
...
Deterministic Playwright tests belong in CI: fast, stable, good at mocked UI
states. They do not answer whether the *deployed* page still behaves correctly
with real data, routing, copy, and latency. Production-like environments are
non-deterministic — data changes, flags drift, third parties fail — so live QA
needs judgment and evidence, not only assertions. This tool is the first-pass
live judgment layer, and it escalates whatever it cannot settle.
...
- **Credentials stay out of the prompt.** `login` stores a session in an
  owner-only profile (`.private/qa-browser-profile`, mode `0700`); `judge`
  relaunches it headless and hands the agent only a CDP URL. The prompt, the
  model context, and the artifacts never see the password or the account email.
- **The CDP window is real and bounded.** While the agent runs, that endpoint is
  loopback-bound but *unauthenticated* — any process running as your user could
  attach. Chromium offers no CDP auth, so the mitigations are the loopback bind and
  ending the browser with the run. `--credentials-in-prompt` opts back into the
  legacy flow, and warns.
- **Origin pinning.** `allowedOrigins` defaults to the staging origin; a
  navigation off it is aborted (or flagged from the HAR) and floors the verdict at
  `fail` with cause `HARNESS_DEFECT`. On a read-only plan, non-GET requests are
  blocked or flagged the same way.
- **Prompt-injection demarcation.** Plan content sits between explicit
  `<<<QA-PLAN-DATA:BEGIN/END>>>` markers declared as data, marker-shaped text inside
  it is stripped, and injection-shaped accessible names are annotated in the aria
  snapshot rather than deleted, so the reviewer sees the attack.
- **The harness captures the evidence, not the agent.** Trace, HAR, screenshots, and
  aria snapshots come from the Playwright context the runner owns, so the thing being
  audited cannot forge or omit them. Secrets are redacted from every query and
  raw-output artifact, and Slack escapes what the agent wrote.

## Limits

Not a replacement for deterministic Playwright CI, API contract tests, or a QA
engineer. It cannot catch backend regressions with no user-facing effect, it is not
a deterministic production test runner, and it is unsafe for destructive flows
unless they are marked `skip` or a `blocked-*` policy. Judgments carry model
non-determinism — verdict history and `--samples=` exist because one run is not proof.

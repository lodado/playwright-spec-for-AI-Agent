...
export function buildJudgeReviewHermesQuery({
  ...
  return [
    ...
    "- Example of **too pedantic:** failing because the template name is exactly `세금계산서` when the intent was only that **a template is visible**.",
    "- Example of **appropriate:** failing when a required control is missing or clearly broken.",
    "- **pass** = intent was respected; **concern** = some checks look nitpicky; **fail** = multiple checks confuse mock literals with user intent.",
    "",
    "## Output rules",

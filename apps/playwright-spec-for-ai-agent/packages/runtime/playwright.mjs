...
import { advanceAdaptiveMilestone, createAdaptiveActionAuthorizer, observationSettleBudget, providerCapabilities } from "./index.mjs";
...
export async function openPlaywrightBrowserToolGateway({
  ...
  if (!Number.isInteger(viewport?.width) || viewport.width <= 0 || viewport.width > MAX_VIEWPORT_DIMENSION || !Number.isInteger(viewport?.height) || viewport.height <= 0 || viewport.height > MAX_VIEWPORT_DIMENSION || viewport.width * viewport.height > MAX_VIEWPORT_AREA) throw new TypeError("viewport is outside gateway limits");

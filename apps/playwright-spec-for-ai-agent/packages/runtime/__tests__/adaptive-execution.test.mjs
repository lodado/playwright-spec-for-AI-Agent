...
describe("adaptive execution authorization", () => {
  ...
  it("preserves a code-owned startup query while rejecting fragments", () => {
    const withQuery = adaptiveQaIr();
    withQuery.suites[0].scenarios[0].steps[0].target = { type: "PATH", value: "/dashboard?status=complete" };
    expect(createAdaptiveExecutionInput({ qaIr: withQuery, scenarioId: "scenario-settings", baseUrl: "https://example.test", runId: "run-query" }).currentPage.url).toBe("https://example.test/dashboard?status=complete");

    const withFragment = adaptiveQaIr();
    withFragment.suites[0].scenarios[0].steps[0].target = { type: "PATH", value: "/dashboard#unsafe" };
    expect(() => createAdaptiveExecutionInput({ qaIr: withFragment, scenarioId: "scenario-settings", baseUrl: "https://example.test", runId: "run-fragment" })).toThrow(/fragment/);
  });
  ...
  it("keeps applicability out of the bounded action goal while passing flow and claims", () => {
    const qaIr = semanticJudgmentQaIr();
    qaIr.suites[0].scenarios[0].title = "Given a signed-in user has settings, open settings";
    qaIr.extensions.abstractScenarios = {
      "scenario-settings": {
        given: ["a signed-in user has settings"],
        when: ["the user opens settings"],
        then: ["the settings dialog is visible"],
    ...
    expect(input.goal.description).not.toContain("Given a signed-in user");
    expect(input.goal.description).not.toContain("a signed-in user has settings");

...
describe("Playwright browser tool gateway", () => {
  ...
  it("closes browser execution before the independent Judge returns a verdict", async () => {
    ...
    const judgment = await judgeEvidence({
      ...
      semanticJudge: async semanticInput => ({
        schemaVersion: SEMANTIC_JUDGE_DECISION_VERSION,
        expectationResults: semanticInput.expectations.map(expectation => ({ expectationId: expectation.id, status: "MATCHED", confidence: 1, evidenceRefs: [semanticInput.evidence[0].id], rationale: "The sealed evidence supports the expectation." })),
        uncertainty: [],
        judge: { provider: "fixture-ai", model: "judge", promptVersion: "judge/3" },
      }),
    });
    expect(judgment.verdict).toBe("PASS");
    expect(judgment.judge.provider).toBe("fixture-ai");
  });

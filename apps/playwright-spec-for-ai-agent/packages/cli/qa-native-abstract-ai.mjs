...
export async function abstractSpecInputs({ specPath, specPaths, sourceInputs, page, cwd }, overrides = {}) {
  ...
  const inputs = rawInputs.map(input => ({ ...input, manifest: input.manifest ?? extractPlaywrightStaticManifest(input) }));
  const results = [];

  for (const [index, input] of inputs.entries()) {
    overrides.progress?.({ index, total: inputs.length, sourcePath: input.sourcePath });
...
export function renderAbstractMarkdown(record) {
  ...
  if (artifact.status !== "APPROVED") {
    ...
    const issues = [...new Set(artifact.attempts.flatMap(attempt => attempt.issues ?? []))];
    if (issues.length > 0) lines.push("## Independent review feedback", "", ...issues.map(issue => `- ${markdownText(issue)}`), "");
    return lines.join("\n");
  }

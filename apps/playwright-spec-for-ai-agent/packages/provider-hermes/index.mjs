...
export const FULL_SPEC_ABSTRACTION_PROMPT_VERSION = "hermes-playwright-full-spec-abstraction/0.17";
export const FULL_SPEC_REVIEW_PROMPT_VERSION = "hermes-playwright-full-spec-review/0.16";
...
export function buildHermesFullSpecAbstractionQuery(input) {
  const query = [
    ...
    "For each structured test source slice and testId return applicability conditions, authored flow in when, observable claims, and classification.",
    "Preserve observable semantics declared by the test title and nearby authored comments even when the assertion checks only a proxy. Preserve exact initial route, account, product-state, count, or retained-state values in applicability only when they are observable before the flow and materially determine the expected outcome.",
    "Applicability contains only live conditions that must already hold and can be observed before the authored when-flow. Do not put future mocked responses, route-handler payloads, fixture identities, dialog/request state created by the flow, assertions, or post-action outcomes in applicability; preserve them exactly in when or claims instead. Do not copy shared setup into a test when its association is ambiguous or changing it would not change that test's own claim.",
    "Example: an initially visible summary claim for counts 2051/567/6 requires those counts in applicability. A 402 response, upload fixture, destination URL, toast, or request emitted after an action belongs in when or claims because a read-only preflight cannot observe it yet.",
    "applicability and when are required semantic descriptions, not executable action programs. classification is a required planning label, not a runtime PASS/FAIL verdict.",
    ...
    "Allowed output is exactly {\"status\":\"ABSTRACTED\",\"tests\":[{\"testId\":\"copy-exactly-from-manifest\",\"applicability\":[\"...\"],\"when\":[\"...\"],\"claims\":[\"...\"],\"classification\":\"LIVE_EXECUTABLE\"}]} or {\"status\":\"MANUAL_REVIEW\",\"reason\":\"...\"}.",
...
export function buildHermesFullSpecReviewQuery(input) {
  const query = [
    ...
    "Treat static manifest identities, ranges, annotations, policies, fixtures, and page values as immutable facts. Check exact one-to-one manifest testId coverage, applicability, authored flow, observable outcome completeness, and classification. Reject invented meaning and loss of network absence, endpoint, payload, count, order, or retained-state semantics.",
    "Check observable semantics declared by each test title and nearby authored comments, not assertions alone. Require exact initial route, account, product-state, count, or retained-state values in applicability only when observable before the flow and material to the expected outcome.",
    "Reject irrelevant or ambiguously associated shared setup copied into applicability. Reject future mocked responses, route payloads, fixture identities, intermediate actions, and post-action states in applicability; preserve those exact semantics in when or claims instead.",
    ...
    "Review each structured test source slice against its candidate entry before approving. applicability, when, and classification are required semantic fields; do not request their removal. classification is not a runtime verdict.",
    "Reject MOCK_ONLY classification based only on page.route, MSW, fixtures, mock constants, or exact stubbed product values. Product state and boundary values belong in applicability. Accept MOCK_ONLY only when the observable claim itself needs test-harness-only evidence, a forced failure unavailable in live QA, or a test-local capture that live evidence cannot reproduce.",
